...
import { Plus, Search, Truck, ChevronRight, ArrowDownLeft, Pencil, Trash2, ScanLine, Loader2 } from 'lucide-react'
import api from '../api'
      ...
      {embedded ? (
        <>
          <div className="rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 text-white p-5 mb-4">
            <div className="flex items-center gap-2 text-rose-100 text-sm"><Truck size={16} /> Total Payable</div>
            <p className="text-4xl font-extrabold mt-1">PKR {totalPayable.toLocaleString()}</p>
            <p className="text-rose-100 text-sm mt-1">{suppliers.length} supplier{suppliers.length === 1 ? '' : 's'} you owe</p>
          </div>
        ...
        {filtered.map(s => (
          <div key={s.id} className="card p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="text-right flex-shrink-0">
                {Number(s.payable_balance) > 0
                  ...
                  : Number(s.payable_balance) < 0
                    ? <div><p className="text-emerald-600 font-bold text-sm whitespace-nowrap">PKR {Math.abs(Number(s.payable_balance)).toLocaleString()}</p><p className="text-xs text-emerald-500">advance</p></div>
            ...
            <div className="flex justify-end gap-0.5 mt-2 pt-2 border-t border-gray-50 flex-wrap">
              <button onClick={() => openPay(s)} className="p-2 rounded-lg hover:bg-emerald-50 text-gray-400 hover:text-emerald-600" title="Record payment (deposit)">
                <ArrowDownLeft size={16} />
              </button>
              <button onClick={() => openBill(s)} className="p-2 rounded-lg hover:bg-amber-50 text-gray-400 hover:text-amber-600" title="Add bill / credit">
                <Plus size={16} />
              </button>
            ...
            {ledger.map(l => (
              <div key={l.id} className="flex items-center justify-between text-sm border-b border-gray-50 py-2 gap-2">
                <div className="min-w-0">
                  <p className="capitalize font-medium text-gray-700">{l.type === 'bill' ? 'Bill / credit' : l.type === 'payment' ? 'Payment' : 'Adjustment'}</p>
                  <p className="text-xs text-gray-400 truncate">{new Date(l.created_at).toLocaleString('en-PK')}{l.note ? ' · ' + l.note : ''}</p>
                </div>
                <div className="text-right flex-shrink-0">
